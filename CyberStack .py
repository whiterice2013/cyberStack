...
for i in range(20):
    for j in range(20):
        box = Entity(color=color.white, model='cube', position=(j, 0, i),
                     texture='grass', parent=scene, origin_y=0.5, collider='box')
        boxes.append(box)
...
def is_too_close_to_player(position):
    return abs(player.position.x - position.x) < 1 and abs(player.position.z - position.z) < 1
    ...
    for box in boxes:
        if box.hovered:
            ...
            if key == 'right mouse down':  # Place a block
                new = Entity(color=color.white, model='cube', position=box.position + mouse.normal,
                             texture='grass', parent=scene, origin_y=0.5, collider='box')
                boxes.append(new)
